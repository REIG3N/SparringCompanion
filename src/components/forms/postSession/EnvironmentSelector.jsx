...
export const EnvironmentSelector = ({ value, onChange, disabled = false, required = false, optional = false, isInvalid = false }) => (
  <View style={{ marginBottom: SPACING.md }}>
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 }}>
      ...
      <Text style={globalStyles.labelText}>{i18n.t('session.form.environment.label')}{!required && optional ? ' (optional)' : ''}</Text>
    </View>
    <RadioPills
      options={[i18n.t('session.env.solo'), i18n.t('session.env.group')]}
      selected={value}
      onSelect={onChange}
      disabled={disabled}
    />
